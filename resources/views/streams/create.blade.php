        ...
        <div class="pcoded-content">
            ...
            <script type="text/javascript">
                    ...
                    switch (cityName) {
                        ...
                        case 'table':
                            ...
                            tablevalues.forEach((value,key)=>{
                                if(!table_name){
                                    $("#table_name").val(tablevalues[key].table_name);
                                }
                                console.log("value",value);
                                console.log("key",key);
                                let  html =   '<tr class="ui-sortable-handle table_rows_count" id="table'+ value.orderCount +'">'
                                html += '<td scope="row"> ' + value.fieldName + '</td>'
                                html += '<td> ' + value.type + '</td>'
                                html += '<td class="index"> ' + value.orderCount + '</td>'
                                html += '<td>'
                                html += '<div class="btn-group" role="group" aria-label="Basic example">'
                                html += '<button type="button" class="btn table_btn  update_btn text-white" onclick="updateTableField(' + value.orderCount + ')" > Update </button>'
                                html += '<button type="button" class="btn  table_btn delete_btn text-white" onclick="removeFieldFromTableList(' + value.orderCount + ')">Delete</button>'
                                html += '</div>'
                                html += '</td>'
                                html += '</tr>';
                ...
                const addField = () => {
                    ...
                    console.log('field_ordercount ',tableData);
                    let data={
                        fieldType,
                        fieldName,
                        fieldOptions,
                ...
                const addTableField = () => {
                    ...
                    let commulative_value = $("input[name=table_cumulative_value]").val();
                    ...
                    for (let i = 0; i < tableData.length; i++) {
                        ...
                        if (tableData[i]?.fieldName == fieldName && !tableHiddenId){
                            toastr.error('This field is already exist');
                            return false
                        }
                    }
                ...
                const updateTableField = (orderCount) => {
                    ...
                    $("input[name=table_cumulative_value][value="+data.commulative_value+"]").prop("checked",true);
