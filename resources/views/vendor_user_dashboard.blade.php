                            ...
                            <div class="form-group pl-4 pt-1 d-flex search_bar_adj">
                                <select class="form-control form-select white_input" name="period_id" aria-label="Default select example">
                                    ...
                                    @foreach($periods as $period)
                                        <option value="{{$period->id}}" {{($current_period_id ? $current_period_id : request()->get('period_id')) == $period->id?"selected":""}}>{{$period->name}} ({{date('d-m-Y', strtotime($period->start_date))}} - {{date('d-m-Y', strtotime($period->end_date))}})</option>
                                    @endforeach
                                </select>
